...
+ (void)dw_ShowPromptAnimation:(UIView *)view imageName:(NSString *)imageNames imageCount:(int)imageCount imageType:(NSString *)imageType maskView:(BOOL)maskView {
    ...
    animationPictures = [[UIImageView alloc] initWithFrame:CGRectMake((animationPicturesView.frame.size.width-80)/2, (animationPicturesView.frame.size.height-64-130)/2, 50, 50)];
...
//    animationPictures.clipsToBounds = YES;
    ...
    for (int i = 0; i < imageCount; i++) {
        ...
        NSString *imageName = [NSString stringWithFormat:@"%@%d",imageNames,i];
