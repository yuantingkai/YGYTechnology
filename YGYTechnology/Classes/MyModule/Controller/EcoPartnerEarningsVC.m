...
#import "ShowEcoEarningsModel.h"
#import "WithdrawDeatilsCell.h"
#import "WithdrawDeatilsModel.h"

@interface EcoPartnerEarningsVC ()<UITableViewDelegate,UITableViewDataSource>
...
@property (nonatomic, strong) WithdrawDeatilsPageData * withdrawPageData;
@property (nonatomic, strong)  UIImageView *noDataImageView;
@end
static NSString * const ecoIdentifier = @"ecoPartnerEarningsCell";
static NSString * const withdrawIdentifier = @"WithdrawDeatilsCell";
@implementation EcoPartnerEarningsVC
...
-(void)setUI{
    ...
    UILabel *showLabel = [UILabel newWithText:@"账户余额（BTC）" fontSize:12 textColor:[UIColor whiteColor] textAlignment:NSTextAlignmentCenter];
    ...
    self.SDTScoreLabel = [UILabel newWithText:@"0.00000000" fontSize:35 textColor:[UIColor whiteColor] textAlignment:NSTextAlignmentCenter];
    ...
    self.earningSumLabel = [UILabel newWithText:@"总收益：0.00000000" fontSize:12 textColor:[UIColor whiteColor] textAlignment:NSTextAlignmentCenter];
    ...
    self.getSumLabel = [UILabel newWithText:@"总提现：0.00000000" fontSize:12 textColor:[UIColor whiteColor] textAlignment:NSTextAlignmentCenter];
...
-(void)clickCenterRightBtn{
    ...
    [self withdrawDeatilsListRequest];
}
-(void)returnTopVC {
...
{
    if ([tableView isEqual:self.tableView]) {
        EcoPartnerEarningsCell *cell = [tableView dequeueReusableCellWithIdentifier:ecoIdentifier];
        if (cell == nil) {
            cell = [[EcoPartnerEarningsCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ecoIdentifier];
        }
        ...
        WithdrawDeatilsCell *cell = [tableView dequeueReusableCellWithIdentifier:withdrawIdentifier];
        if (cell == nil) {
            cell = [[WithdrawDeatilsCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:withdrawIdentifier];
        }
        if (self.withdrawDataSourceArr.count > 0) {
            self.withdrawPageData = self.withdrawDataSourceArr[indexPath.section];
            [cell setIndexPath:indexPath withPageData:self.withdrawPageData];
        }
...
//收益明细列表请求
-(void)getEarningsListRequest{
    LOADING
    [self removeNoDataImage];
    self.refreshNum = 10;
    NSString *urlStr = [NSString stringWithFormat:@"%@userId=%@&index=1&size=%ld",GET_EARNING_LIST,USER_ID,self.refreshNum];
    urlStr = [urlStr stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
    [YGYBaseViewModel getRequestWithPath:urlStr parameters:nil responseDataModel:[EcoEarningsModel class] success:^(id responseObject) {
        STOP_LOADING
        EcoEarningsModel *model = responseObject;
        EcoEarningsData *data = model.data;
...
-(void)getEarningsRequest{
    NSString *urlStr = [NSString stringWithFormat:@"%@uid=%@",GET_EARNING_URL,USER_ID];
    urlStr = [urlStr stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
    [YGYBaseViewModel getRequestWithPath:urlStr parameters:nil responseDataModel:[ShowEcoEarningsModel class] success:^(id responseObject) {
        ShowEcoEarningsModel *model = responseObject;
        ShowEcoEarningsData *data = model.data;
        if ([model.status isEqualToString:@"200"]) {
            self.SDTScoreLabel.text = [NSString stringWithFormat:@"%.8f",[data.balance doubleValue]];
            self.earningSumLabel.text = [NSString stringWithFormat:@"总收益:%.8f",[data.incomeTotal doubleValue]];
            self.getSumLabel.text = [NSString stringWithFormat:@"总提现:%.8f",[data.withdrawTotal doubleValue]];
        }else{
            MBHUD(model.msg);
        }
...
//提现明细列表请求
-(void)withdrawDeatilsListRequest{
    LOADING
    [self removeNoDataImage];
    self.refreshNum = 10;
    NSString *urlStr = [NSString stringWithFormat:@"%@userId=%@&index=1&size=%ld",WITHDRAW_DETAILS_LIST,USER_ID,self.refreshNum];
    urlStr = [urlStr stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
    [YGYBaseViewModel getRequestWithPath:urlStr parameters:nil responseDataModel:[WithdrawDeatilsModel class] success:^(id responseObject) {
        STOP_LOADING
        WithdrawDeatilsModel *model = responseObject;
        WithdrawDeatilsData *data = model.data;
        if ([model.status isEqualToString:@"200"]) {
